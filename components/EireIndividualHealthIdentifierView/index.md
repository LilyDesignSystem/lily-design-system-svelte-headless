...
EireIndividualHealthIdentifierView is a headless Svelte 5 display component that shows an Eire Individual Health Identifier (IHI) as read-only text. The IHI is the 10-digit clinical identifier used by the Irish Health Service Executive (HSE) to match patients with their medical records.

## What it is

A thin, unstyled `<span>` wrapper that renders an IHI string with an `aria-label` for additional screen-reader context. It is the read-only companion to `EireIndividualHealthIdentifierInput` in the Input/View pattern.

## What it does

- Renders a `<span>` with `class="eire-individual-health-identifier-view"` plus any consumer-provided CSS class.
...
- Do not use for editing — use `EireIndividualHealthIdentifierInput`.
...
<EireIndividualHealthIdentifierView label="IHI" value="1234567890" />
```
...
<p>
    Patient IHI:
    <EireIndividualHealthIdentifierView label="IHI" value={ihi} />
</p>
```
...
- `EireIndividualHealthIdentifierInput` — paired input for entering the same identifier.
...
- `TuaisceartEireannHealthAndCareNumberView` / `TuaisceartEireannHealthAndCareNumberInput` — Tuaisceart Eireann H&C number.
